...
class ImageViewer:
    ...
    def show_masked_image(self):
        img = cv2.imread(self.file_path)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        masked_image = self.get_masked_image_sigle(img)
        ...
        img = pil_image.resize((width, height), Image.LANCZOS)
    ...
    def get_masked_image_sigle(self, img):
        colored_mask = np.zeros_like(self.mask)
        colored_mask[self.mask == 1] = 125 
    ...
    def create_mask_as_image(self):
        ...
        binary_mask_array[binary_mask_array == 1] = 255
    ...
    def load_masked_points(self):
        ...
        for mm in mask_list:
            ...
            clicked_points_temp = mask_dict["point"]
            clicked_points_temp.append(clicked_points_temp)
            x = clicked_points_temp[0] * self.zoom_factor + self.x_position
            y = clicked_points_temp[1] * self.zoom_factor + self.y_position
            self.canvas.create_oval(x - 2, y - 2, x + 2, y + 2, fill="red")  # Create a red dot
